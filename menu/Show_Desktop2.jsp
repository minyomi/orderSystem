          ...
          <section class="wrapper">
              ...
              <section class="panel">
                          ...
                          <table class="table table-striped table-advance table-hover">
                              ...
                              <tbody>
                                 <%
                                   List<DeskTop> desktops=(List<DeskTop>)request.getAttribute("desktops");
                                   List<DeskGroup> deskgroups=(List<DeskGroup>)request.getAttribute("deskGroups");
                                   Iterator<DeskTop> it1=desktops.iterator();
                                   Iterator<DeskGroup> it2=deskgroups.iterator();
                                   while(it1.hasNext()&&it2.hasNext())
                                     {
    	                              DeskTop desktop=it1.next();
    	                              DeskGroup deskgroup=it2.next();
    	
                                 %>
                              <tr>
                                  <td><%=desktop.getId()%></td>
                                  <td class="hidden-phone"><%=desktop.getName() %></td>
                                  <td><%=desktop.getStatus() %></td>
                                  <td><%=desktop.getCustomer_number() %></td>
                                  <td><%=desktop.getCapacity() %></td>
                                ...
                                <form role="form" class="form-horizontal tasi-form" action="ChangeDeskTop" method="post">
                                <input type="hidden" name="id" value="<%=desktop.getId()%>">
                          ...
                          <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>    
                                      ...
                                      <a href="DeleteDeskTop?id=<%=desktop.getId()%>"><button class="btn btn-danger btn-xs tooltips" data-original-title="删除"><i class="icon-trash "></i></button></a>									
                                  </td>
                              </tr>
